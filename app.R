...
dataPanel_3 <- tabPanel("Price Calculator",
                        ...
                        sidebarLayout(position = "right",
                                      sidebarPanel(
                                        ...
                                        fluidRow(
                                          h3(style = "margin-left: 0px; margin-bottom: 0px;", "Wage (€/year)"),
                                          column(12,
...
server <- function(input, output) {
  ...
  output$Overall <- plotly::renderPlotly({
    ggplot(df, aes(x = .data[[input$select_overall]], y = Overall, color = Position)) + geom_point(size=0.8) + 
  ...
  output$Calculator <- renderText({
    ...
    paste("Hi! In case you are the player transfer manager of a football club, you can now find out how much would it cost a desired player in terms of his stats you are most interested at the moment (tunning them on the right panel).", 
          "<br>","<br>", 
          ...
          "<br>","<br>", 
          "So, after tunning these player stats, the estimated", "<b>", "buying price", "</b>", "for a player with the following characteristics:",
          "<br>",
